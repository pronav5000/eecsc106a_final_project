...
class CupPosePublisher(Node):
    def __init__(self, base_frame, marker_frame):
        ...
        self.get_logger().info(
            f"Publishing cup pose as TF({self.base_frame} → {self.marker_frame})"
        )
...
def main(args=None):
    ...
    if len(sys.argv) != 3:
        print("Usage: ros2 run my_pkg cup_pose_publisher base_frame ar_marker_frame")
        rclpy.shutdown()
        return
    ...
    node = CupPosePublisher(base_frame, marker_frame)
    rclpy.spin(node)
    node.destroy_node()
    rclpy.shutdown()
